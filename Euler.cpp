...
int main(){    
  ...
  for (unsigned int ig = 0; ig < 1; ig++){ 
    ...
    //    cout << ig << endl;
    double dt = 0.001 + 0.003*ig;
    double TimeStop = 30000.0;
    double time = 0.0;


    ostringstream Trajname;
    Trajname << "Datafiles/TEST2_Traj_cc"; Trajname << ig; Trajname << ".txt";
    std::ofstream Trajfile (Trajname.str().c_str());

    ostringstream Newname;
    Newname << "Datafiles/TEST2_Poin_cc"; Newname << ig; Newname << ".txt";
    ...
    std::mt19937 gen(rd());
    //    std::normal_distribution<> d(0.0 , 0.000801);
    std::normal_distribution<> d(0.0 , 3.000001);

    std::random_device rd2;
    std::mt19937 gen2(rd2());
    //    std::normal_distribution<> d2(0.0 , 0.00008);
    std::normal_distribution<> d2(0.0 , 0.300001);
